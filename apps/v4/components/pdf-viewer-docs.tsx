...
function SearchInput({
  ...
  const [value, setValue] = useState(() => searchStore.getDraftSnapshot());
...
function Component({ url }: { url: string }) {
  ...
  const thumbnailClickScrollYRef = useRef<number | null>(null);
  const viewportRef = useRef<HTMLDivElement>(null);
  const searchStore = useMemo(() => createSearchStore(), []);
  ...
  const updateCurrentPageFromViewport = useCallback(() => {
    ...
    viewport.querySelectorAll<HTMLElement>("[data-page-number]").forEach((page) => {
  ...
  function preserveThumbnailClickScroll(event: React.PointerEvent | React.MouseEvent) {
...
export function PdfViewerSource() {
  return <HighlightedCodeBlock code={pdfViewerUsageCode} />
}
