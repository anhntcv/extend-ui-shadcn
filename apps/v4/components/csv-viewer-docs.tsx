        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground">
              ...
              {parsed.headers.length.toLocaleString()} columns
            </div>
          </div>
        </div>
        <TooltipProvider>
...
export function CsvViewer({
  csvText = SAMPLE_CSV,
  fileName: initialFileName = "invoice-review.csv",
  className,
}: CsvViewerProps) {
  const inputRef = React.useRef<HTMLInputElement | null>(null);
  const isDark = useIsDarkTheme();
  ...
  const [zoom, setZoom] = React.useState<(typeof ZOOM_OPTIONS)[number]>(1);
  const [parsed, setParsed] = React.useState(() => parseDelimitedText(csvText));
  const [isPending, setIsPending] = React.useState(false);
  ...
  return (
    <div
      className={cn(
        "flex h-[560px] w-full flex-col overflow-hidden rounded-xl border bg-background",
        className,
      )}
    >
      <div className="flex min-h-12 items-center justify-between gap-3 border-b px-3">
...
export function CsvViewerSource() {
  return <HighlightedCodeBlock code={csvViewerUsageCode} />
}
