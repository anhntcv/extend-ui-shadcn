...
const XLSX_URL = withUiBasePath("/samples/crazy-chart-zoo.xlsx")
...
const PageThumbnailPreview = React.memo(function PageThumbnailPreview({
  ...
  return (
    ...
    >
      <button
        type="button"
        className={THUMBNAIL_BUTTON_CLASS_NAME}
        style={{ width: thumbnailSize.width, height: thumbnailSize.height }}
...
function PageDragOverlay({
  ...
  return (
    <div
      className={cn(THUMBNAIL_SURFACE_CLASS_NAME, "shadow-lg shadow-black/10")}
      style={{ width: thumbnailSize.width, height: thumbnailSize.height }}
    >
      <FileThumbnail
        ...
        <div className="flex w-max gap-2 overflow-y-hidden py-1 pr-4">
          {pages.map((page) => (
            <button
              key={page}
              type="button"
              className="relative h-[92px] w-[72px] shrink-0 overflow-hidden rounded-lg border border-border bg-background text-xs text-foreground shadow-xs transition-[border-color,box-shadow] outline-none hover:border-ring/40 hover:ring-2 hover:ring-ring focus-visible:border-ring/40 focus-visible:ring-2 focus-visible:ring-ring"
            >
...
export function DocumentSplitsDemo() {
  return (
    ...
    >
      <DocumentSplitsPreview file={PDF_URL} />
